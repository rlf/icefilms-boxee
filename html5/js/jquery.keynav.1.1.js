...
$.fn.keynav = function(onClass, offClass) {
	...
	return this.each(function() {
		$.keynav.reg(this, onClass, offClass, $(this));
	});
}
...
$.keynav.reg = function(e, onClass, offClass, th) {
	var kn = $.keynav;
	e.pos = $.keynav.getPos(e, th);
	e.onClass = onClass;
	e.offClass = offClass;
...
$.keynav.activateClosest = function(cur, quad) {
	...
	for( i = 0; i < quad.length; i++) {
		...
		// Only parents that's actually being shown...
		if(nd < od && e.jObj.parents('.shown').size() > 0) {
			closest = e;
			od = nd;
			found = true;
		}
...
$.keynav.getPos = function(e, th) {
	...
	//console.log('getPos(' + e + ') = ' + l + ',' + t + ',' + w + 'x' + h);
	return {
		x : l,
		y : t,
		w : w,
